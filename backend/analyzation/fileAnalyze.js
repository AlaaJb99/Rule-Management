const { getRules,addRule } = require('../database_utils/rules_utils');
const logAnalyze = require('./logAnalyze');
const { getLogs } = require('../database_utils/file_utils');


//log_file is the name of the collection
function fileAnalyze(log_file) {
    ...
    (async () => {
        try {
            ...
            //console.log(rules);
            const logs = await getLogs(log_file);
            //console.log('Logs as an array:', logs);

            for (var i = 0; i < logs.length; i++) {
                process.push(logAnalyze(logs[i], rules));
            }
            console.log(process);
